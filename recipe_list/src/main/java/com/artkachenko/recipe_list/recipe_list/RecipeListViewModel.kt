...
class RecipeListViewModel @Inject constructor(private val recipeRepository: RecipeRepository) : ViewModel(),BaseViewModel by BaseViewModelImpl() {
    ...
    fun getRecipeList() {
        scope.launch {
            ...
            val vegetarianRecipes = recipeRepository.getRecipeList(0, *Filters.italianPreset.toList().toTypedArray(), Filters.fiveItemPreset)
            _recipes.emit(State.Vegetarian(vegetarianRecipes))
            val indianRecipes = recipeRepository.getRecipeList(0, *Filters.italianPreset.toList().toTypedArray(), Filters.fiveItemPreset)
            _recipes.emit(State.Indian(indianRecipes))
            val quickRecipes = recipeRepository.getRecipeList(0, *Filters.italianPreset.toList().toTypedArray(), Filters.fiveItemPreset)
